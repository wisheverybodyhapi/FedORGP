...
class clientKD(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        if self.save_folder_name == 'temp' or check_for_model(self.model_folder_name) == False:
            self.W_h = nn.Linear(args.feature_dim, args.feature_dim, bias=False).to(self.device)
            self.global_model = BaseHeadSplit(args, 0).to(args.device)
        else:
            self.W_h = load_item(self.role, 'W_h', self.model_folder_name)
    ...
    def train(self):
        ...
        optimizer_W = torch.optim.SGD(self.W_h.parameters(), lr=self.learning_rate)
        ...
        self.compressed_param = decomposition(self.global_model.named_parameters(), self.energy)

        self.train_time_cost['num_rounds'] += 1
        self.train_time_cost['total_cost'] += time.time() - start_time

        
    def set_parameters(self, compressed_param):
